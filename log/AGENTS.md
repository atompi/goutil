...
```
log/
├── common.go    # 文件写入器工厂 + 路径验证
├── option.go    # 配置结构体 + 函数式选项
...
- 无自动日志轮转（文件无限增长）
- `MultiFiles=true` 会创建单独的 `.debug.log`、`.info.log`、`.warn.log`、`.error.log`

## 符号表

| 符号 | 类型 | 位置 | 说明 |
|------|------|------|------|
| Logger | 结构体 | option.go:3 | 配置容器 |
| Options | 类型 | option.go:10 | 函数式选项类型 |
| NewSlogLogger | 函数 | slog.go:68 | 构造函数 |
| NewZapLogger | 函数 | zap.go:11 | 构造函数 |
| NewLogFile | 函数 | common.go:19 | 文件写入器工厂 |
